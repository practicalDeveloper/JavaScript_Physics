...
//   // function to cancel the animation timer
//   this.cancelTimer = function (value) { 
...
//     let cancelTimer = this.cancelTimer.bind(this);
...
function SpringDynamometer(options = {}) {
  ...
  this.fields = {
    ...
    getMeasures: function () {
      ...
      let springH = this.height - this.height / 3;
      // let holderL = this.length - (springL + swingWidth);
      let holderL = this.length - swingWidth + this.length / 20;
      ...
      //let hookX = initialLeftX + Math.cos(application.degrToRad(this.angle)) * (springValueL + swingWidth + holderL);
      //let hookY = initialLeftY + Math.sin(application.degrToRad(this.angle)) * (springValueL + swingWidth + holderL);
      let hookX = initialLeftX + Math.cos(application.degrToRad(this.angle)) * (swingWidth + holderL);
      let hookY = initialLeftY + Math.sin(application.degrToRad(this.angle)) * (swingWidth + holderL);
...
SpringDynamometer.prototype = {
  ...
  draw: function () {
    ...
    let lineY = this.startY + measures.springH;
    let lineX = this.startX + measures.springValueL + measures.swingWidth;
    //let lineX = this.startX +  measures.swingWidth;
    ctx.moveTo(lineX, lineY);
    ctx.lineTo(lineX + measures.holderValueL, lineY);
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.restore();
