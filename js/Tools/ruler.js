...
function Ruler(options = {}) {
    ...
    Object.assign(this, {
      ...
      showMinorLines : true, //shows/hides minor lines
  ...
  Ruler.prototype.draw = function () {
    ...
    let font = this.height * 0.3 + "px Times New Roman";
    ctx.beginPath();
    ctx.strokeStyle = this.strokeColor;
    ...
    for (let x = 1; x <= (this.maxValue * 10) - 1; x ++) {
      ...
      //long lines
      {
          ...
          let drawText = () => contextLayout.drawCenterText(ctx, font, x/10, xCoord, this.startY + this.height /2);
          
          if(!this.showOddMarks){
            if((x/10) % 2 != 0){
              drawText();
            }
          }
      ...
      {
          lineHeight = this.showMinorLines ? this.height /6 : 0;
      }
